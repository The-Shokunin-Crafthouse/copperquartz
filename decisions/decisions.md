...
## 2026-05-07 — Add `--color-ink-deep` to v1 token map for Registry overlay panels
**Stage:** 03-build
**Status:** accepted

**Context.** Sprint 5 (Registry) Figma frame `22:1243` renders the three reveal-overlay panels (Honeymoon, Howlin' Dog, Kiva) on a near-black deep-teal field `#0E2122`. The v1 token map has no surface color in this register — `--color-brown` (#5D4931) is too warm/tan, `--color-palm-leaf` darkened drifts hue, and improvising a derived value silently creates a one-off. The studio rule (`WORKSPACE.md §4`) is to surface ambiguity, not resolve it silently.
**Decision.** Add a single new color token `--color-ink-deep: #0E2122` to `_config/design-system/token-map.md` and `src/styles/tokens.css`. Use is scoped to content-on-image surfaces where contrast must dominate (Registry tile reveal panels). It is not promoted as a generic surface color, foreground color, or button fill — pages that need a deep field reach for this token explicitly with that constraint in mind.
**Rationale.** The overlay panels carry the only CTA on the Registry page; their text legibility is non-negotiable. `#0E2122` paired with `--color-sand-linen` (#F3E6D3) clears AA at all sizes (contrast ≈ 14.7:1). Re-using a hue-mismatched existing token would have introduced visible drift from Figma without saving anything — wedding-site palettes are small enough that a precision-named addition is cheaper than a derived approximation. The token is nominal (named by hue intensity, not semantic role), consistent with v1's nominal palette discipline.
**Consequences.** v2 semantic-alias layer (already contemplated in token-map header) will likely fold this into something like `--color-overlay-on-media` or `--color-fg-strong`. Until then, the nominal name documents the value and its scope-of-use is constrained by inline comments in `tokens.css` and the use column in `token-map.md`. If a second surface in the project legitimately needs the same hue, this entry is the precedent — not improvising again.

## 2026-07-08 — Verify Resend acceptance before recording a digest run
**Stage:** 03-build
**Status:** accepted

**Context.** The daily RSVP digest (`src/app/actions/sendRsvpDigest.ts`, Vercel cron `0 13 * * *` → `/api/cron/rsvp-digest`) stopped landing in the inbox after 2026-06-14, yet `digest_runs` kept recording rows (Jun 20 → Jul 8, `responses_included ≥ 1`) with no runtime errors and no code deploy in the gap. Diagnosis at the destination (Gmail `in:anywhere`, both recipients): last delivered digest is Jun 14 13:12; nothing since. Two live probe sends through the app's `RESEND_API_KEY` returned HTTP 200 + a message id with `error: null`, and neither arrived. So Resend is *accepting* the API call (200 + id) but not delivering — a post-acceptance drop at the Resend account/domain layer (studio learnings #45/#46). The old code checked only `sendRes.error`; on a genuine 200 it inserted the `digest_runs` row and returned success, advancing the cutoff over responses no human ever saw.
...
## 2026-08-03 — Unreferenced source art lives under `stages/`, not `public/`
**Stage:** 03-build
**Status:** accepted

**Context.** Seventeen files had accumulated untracked in the working tree of a checkout that auto-apply automation cycles branches through — exactly the durability hazard in studio learning #129. A reference sweep of `src/` established that exactly one of them (`public/fonts/hesland-regular.woff2`) is loaded by the site; it is named in the `@font-face` block in `src/styles/globals.css`, so every deployed build had been serving a missing font and falling back silently. The other sixteen split three ways: ten unreferenced originals (`hdmg.png` at 2960×1440, `honeymoon.jpg`, `kiva.jpg`, `howlindog.jpg`, `howling-logo.png`, `2026-02-17.webp`, two honeymoon SVGs, `howling-logo.svg`, and a 13 MB print invitation PDF — about 19 MB sitting inside `public/`), four sha256-identical duplicates of already-tracked files, and two stage documents. The originals are the sources behind the tracked `*-desktop.jpg` / `*-mobile.jpg` derivatives the pages actually load.

**Decision.** Split by whether the site loads the file, not by where it happened to land. The font is tracked in `public/fonts/`. The ten unreferenced originals move to a new `stages/02-design/source-assets/` and are tracked there, including the 13 MB PDF. The two stage documents are committed in place. The four exact duplicates (`public/images/{favicon.ico,favicon-32x32.png,apple-touch-icon.png}`, superseded by the App Router file conventions in `src/app/`, and `honeymoon-dekstop.jpg`, a typo copy of `honeymoon-desktop.jpg`) are left untracked pending a delete confirmation — deleting non-generated files is on the always-ask list even when a hash proves zero loss. Separately, the `.gitignore` guest-list rule was widened to `*guest-list*.xlsx`: the file on disk is `" guest-list.xlsx"` with a leading space, which the bare filename pattern never matched, leaving a spreadsheet of real guest names one `git add -A` away from being committed.

**Rationale.** Studio learning #52 says commit the source for every rendered asset, but `public/` is a deploy surface — anything under it ships to Vercel on every build whether or not a page references it. Tracking the originals under `stages/02-design/` satisfies #52 without paying ~19 MB of transfer per deploy for files no route requests. The stage tree is already the project's home for design-time material, so no new top-level folder and no `workspace.manifest.yaml` change is needed. Keeping the PDF in git is the weaker half of this call: 13 MB is permanent history weight and hard to remove later. It is kept because the print invitation is one-of-a-kind source art for a project that will be archived rather than maintained, and because stranding it in an untracked working tree is the failure #129 describes.

**Consequences.** Any future page that wants the full-resolution originals must generate a derivative into `public/images/` rather than referencing `stages/`. The 13 MB PDF is now permanent unless history is rewritten. The four duplicates stay as untracked noise in `git status` until a delete is confirmed. Both `@font-face` blocks still list a `.woff` fallback `src` with no file behind it — harmless, since woff2 is listed first and universally supported, so the fallback is never fetched, but it remains a dead reference. Landed as PR #58; `npm run build` passes, 13/13 static pages.

## 2026-08-04 — Close the RSVP window behind a positive flag with a private bypass key
**Stage:** 03-build
**Status:** accepted

**Context.** The RSVP deadline has passed and the final count is with the venue, so the public RSVP path has to close. But it cannot close permanently: Levi still needs to reach the wizard himself to adjust a party's count when a guest calls, and the flow has to be screenshot-able later for the portfolio site. Six guests across three parties (Novick, Gabert/Fry, Rider) still have no `rsvp_responses` row, so a hard teardown would also strand any late arrival. Prior art in this repo: `RSVPButton` already accepts a `disabled` prop (unused), and `/rsvp` was fully public with `force-dynamic`.

**Decision.** Two independent levers over one shared rule module, default CLOSED.

1. `src/lib/rsvpWindow.ts` — runtime-agnostic (no `next/headers`, no `node:crypto`) so Edge middleware and Node server components import the same rule. Exports `rsvpPubliclyOpen()` (env only) and `isBypassKey()` (hand-rolled constant-time compare; `timingSafeEqual` is absent on Edge). Refuses any configured key under 16 chars.
2. `src/lib/rsvpAccess.ts` — `server-only`, adds the request-scoped `rsvpAccessible()` = public OR valid bypass cookie.
3. **Lever A — `RSVP_ENABLED=1`:** fully public. Nav + footer pills return, `/rsvp` renders the wizard. Requires a Vercel env edit **and a redeploy** (env is inlined at build for the statically-rendered layout — learning #95).
4. **Lever B — `RSVP_BYPASS_KEY`:** `/rsvp/?key=<key>` mints an httpOnly, 12-hour, SameSite=Lax cookie in middleware that unlocks the wizard and both server actions for that browser only. `?key=off` clears it. No deploy, no env edit. Minting lives in middleware because a server-component render cannot set cookies, and middleware is the only layer that sees the query string before the page decides what to render. A wrong key is treated byte-identically to no key — same 307 to the clean URL, no cookie, no distinguishing response — so the endpoint is not an oracle. Both paths strip `key` from the URL on redirect so it never lands in history or a screenshot.
5. **Gated at every layer, not just the UI:** `/rsvp` page, `lookupParty`, and `submitRsvp` all call `rsvpAccessible()`. `lookupParty`'s error union and `StepNameEntry`'s `ERROR_COPY` map both gain `rsvp_closed` so the map stays total over the union.
6. **Closed state is a panel, not a 404** (Levi's call, 2026-08-04). `/rsvp` renders `RsvpClosed.tsx`, reusing the `.confirmation*` CSS vocabulary — same heading, subline, palm break, footer — so it reads as a member of the existing family and adds zero CSS. The URL is printed on the invitation inserts; a guest arriving late gets an explanation and a route to reach us instead of a dead end.
7. **One boundary for the whole route:** closed hides the `?confirmation=` receipt too, so there is a single state to reason about rather than a half-open surface.
8. **Pills are removed, not disabled.** `showRsvp` defaults true and is passed down from the layout. A disabled pill after a passed deadline reads as a broken control; absence reads as finished business. `RSVPButton`'s `disabled`/`aria-disabled` branch stays available but unused here.

**Rationale.** The flag is positive (`RSVP_ENABLED`, not `DISABLE_RSVP`) so an unset or freshly-provisioned env fails shut rather than silently reopening the guest list — learning #73. Gating the actions and not just the button is learning #72: hiding a control while leaving the write live means a crafted POST keeps writing after the deadline. The layout reads the **sync env-only** helper rather than `rsvpAccessible()` on purpose — calling `cookies()` in `(site)/layout.tsx` would force every page in the group to dynamic rendering, trading the whole site's static output for a nav detail. Verified: post-change build keeps `/`, `/our-story`, `/qa`, `/travel`, `/venue`, `/savethedate` as `○ (Static)`. The consequence Levi accepted is that a bypassing admin sees the pills hidden — the bypass is a direct `/rsvp` visit, not a restoration of public chrome. The 12-hour cookie is deliberately short: the closed state is where the site should drift back to without anyone remembering to re-hide it.

**Verification.** Six states exercised against a real `next start`, not inferred from the build: (1) default → closed panel, 0 `/rsvp` hrefs sitewide; (2) wrong `?key` → 307, no cookie, byte-identical to no key; (3) correct `?key` → cookie minted, `HttpOnly`, `Max-Age=43200`; (4) valid cookie → wizard renders; (5) invalid cookie → still closed; (6) `?key=off` → cookie expired to epoch. Reopen path proven by rebuilding with `RSVP_ENABLED=1`: pills return, wizard renders. `tsc --noEmit` and `eslint --max-warnings=0` clean.

**Consequences.** `RSVP_BYPASS_KEY` must be set in Vercel (Production) with a 32+ char random value before Lever B works — until then the site is simply closed with no admin path, and `?key=` anything is inert. Rotating either var needs a redeploy. `/admin` is untouched and remains the read path for the count, so day-to-day checking needs no bypass at all. Toggle runbook: hide = default, no action; privately open = visit `/rsvp/?key=<key>`; re-hide = visit `/rsvp/?key=off` or wait 12h; publicly reopen = set `RSVP_ENABLED=1` + redeploy.

## 2026-09-01 — Publish the Monday meetup as the first, chronological section on /venue
**Stage:** 03-build
**Status:** accepted

**Context.** The Monday-evening meetup at Validation Ale existed on the site only as an RSVP question (`StepMondayMeetup`). The RSVP window closed 2026-08-04 (see that entry), which took the wizard *and* the `?confirmation=` receipt out of reach — so every guest who answered "yes" now has no way to find the time, the address, or the fact that most of the seating is outdoors. Three surfaces could plausibly hold it: `/venue` (already a list of places with time + address + media), `/qa` (markdown-driven FAQ), `/travel` (the early-arrival audience). Levi supplied the copy points, a Framer-hosted mp4, and a Visit Santa Barbara jpg.

**Decision.** One home, two pointers. Full details go in a new `venue-night-before` section on `/venue`, placed **first**, chronologically ahead of the ceremony and reception. `/qa` gains a single Logistics question ("Is anything happening the night before?") whose answer links to that anchor, and `/travel`'s Local Recommendations intro gains one sentence linking to the same anchor. The photo is used as the `VideoFrame` poster rather than as a second media slot.

**Rationale.** `/venue` already carries exactly this payload — heading → `.meta` rows → media → body — so the section is a verbatim structural clone of the two shipped ones: no new component, no new CSS, and the 2.76:1 crop comes free from `VideoFrame` (see 2026-05-06 — VideoFrame is the only path for in-page video). Duplicating the details into the FAQ would have created two copies to keep in sync; the FAQ already establishes the link-out pattern ("Head to the Travel page…"), so it was reused. Chronological placement is Levi's call, taken with the trade-off named: it puts a brewery meetup above the ceremony on the venue page. Poster-as-still was chosen over a stacked photo because every other section on the page holds exactly one media slot, and over photo-only because the section would then be the one static block on a page of moving ones. The address link uses a `maps.google.com/?q=` query URL rather than a `maps.app.goo.gl` short link like its neighbours — short links can only be minted from the Maps UI, and the query form resolves identically.

**Consequences.** The meetup now has one canonical location; changing the time or venue means editing `venue/page.tsx` and nothing else. `/venue`'s `metadata.description` and its header comment now describe three sections and a two-day span, so future edits to that page inherit the chronological framing. Two follow-ups are deliberately **not** in this change: (1) `VideoFrame`'s block comment claims the inline video "renders paused with poster" under `prefers-reduced-motion`, but `autoPlay` is unconditional — a pre-existing defect across all five clips, logged here and left for its own pass rather than smuggled into a content change (studio learning #69); (2) the media assets themselves could not be fetched by any tool in the session — both hosts are refused by the org egress allowlist from the cloud container *and* the desktop VM, and Chrome refuses a scripted save — so acquiring them is a manual step, and `validation-ale.mp4` / `validation-ale.jpg` must exist under `public/` before the preview reads true. Verification runs on CI and the Vercel PR preview rather than locally: `next build` cannot complete inside the 120-second device-shell ceiling on the FUSE-mounted checkout, and Google Fonts is likewise refused by the allowlist, so any container-side screenshot would render fallback type and prove nothing about the typography.
